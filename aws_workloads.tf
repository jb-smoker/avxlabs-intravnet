...
module "ec2_instance_wp_web" {
  ...
  user_data              = templatefile("${path.module}/aws_web.tftpl", { db_fqdn = "${aws_db_instance.wp_mysql.address}", db_user = "${var.database_admin_login}", db_password = "${var.database_admin_password}", wp_alb_url = "${aws_lb.aws_wordpress_prod.dns_name}" })
